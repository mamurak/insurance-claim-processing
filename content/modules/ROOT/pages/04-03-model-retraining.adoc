...
== Re-train the model within a pipeline

Notebooks are great for prototyping, but long-running processes like model training jobs are best executed as workflows or pipelines. Explore the contents of folder `03-training-pipeline`, which contains a pipeline version of the model re-training notebook.