...
We're now going to provision a workbench environment, in which you will interactively work on the lab exercises. To do this:
...
* You should not need to modify Storage.
* Under `Data Connections`, choose `Use a data connection`, `Use existing data connection`, and select the `claims-images` data connection.
...
[subs=attributes+]
----
https://github.com/mamurak/insurance-claim-processing.git
----
+
[.bordershadow]
