...
def ingest_data(bucket_name='', bucket_folder='training_images/', data_folder='./data'):
    ...
    for page in pages:
        for obj in page.get('Contents', []):
            key = obj['Key']
            if key.startswith(bucket_folder):
                relative_object_path = key.removeprefix(bucket_folder)
                local_file_path = path.join(data_folder, relative_object_path)

                subdirectory = dirname(local_file_path)
...
if __name__ == '__main__':
    ingest_data(data_folder='/datasets')
